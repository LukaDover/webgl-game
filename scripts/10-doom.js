...
function initTextures() {
  ...
  wallTexture.image.src = "./assets/grass.jpg";
  ...
  metalTexture.image.src = "./assets/test.jpg";
}
...
//JULIJAN
function handleLoadedtree(treeData) {
  // Pass the normals into WebGL
  treeVertexNormalBuffer = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, treeVertexNormalBuffer);
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(treeData.data.attributes.normal.array), gl.STATIC_DRAW);
  treeVertexNormalBuffer.itemSize = 3;
  treeVertexNormalBuffer.numItems = treeData.data.attributes.normal.array.length / 3;

  // Pass the texture coordinates into WebGL
  treeVertexTextureCoordBuffer = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, treeVertexTextureCoordBuffer);
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(treeData.data.attributes.uv.array), gl.STATIC_DRAW);
  treeVertexTextureCoordBuffer.itemSize = 2;
  treeVertexTextureCoordBuffer.numItems = treeData.data.attributes.uv.array.length / 2;
  ...
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(treeData.data.attributes.position.array), gl.STATIC_DRAW);
  treeVertexPositionBuffer.itemSize = 3;
  treeVertexPositionBuffer.numItems = treeData.data.attributes.position.array.length / 3;
  ...
  gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(treeData.data.index.array), gl.STATIC_DRAW);
  treeVertexIndexBuffer.itemSize = 1;
  treeVertexIndexBuffer.numItems = treeData.data.index.array.length;
}

function loadTree() {
  var request = new XMLHttpRequest();
  request.open("GET", "./assets/test.json");
  request.onreadystatechange = function () {
    if (request.readyState == 4) {
      handleLoadedtree(JSON.parse(request.responseText));
    }
  }
  request.send();
}
...
function drawScene() {
  ...
  if (worldVertexTextureCoordBuffer == null || worldVertexPositionBuffer == null || treeVertexNormalBuffer==null || treeVertexPositionBuffer==null || treeVertexIndexBuffer==null || treeVertexTextureCoordBuffer==null) {
    return;
  }
  ...
  mat4.translate(mvMatrix, [-xPosition, -yPosition-0.5, -zPosition]); //JULIJAN malo sem dol prestavil floor
  ...
  //mat4.rotate(mvMatrix, degToRad(-pitch), [1, 0, 0]);
  mat4.rotate(mvMatrix, degToRad(-yaw), [0, 1, 0]);
  mat4.translate(mvMatrix, [-xPosition+2, -yPosition-1, -zPosition-10]); //JULIJAN malo sem dol prestavil floor
  ...
  // Set the vertex positions attribute for the tree vertices.
  gl.bindBuffer(gl.ARRAY_BUFFER, treeVertexPositionBuffer);
...
//še enkrat
  ...
  mat4.rotate(mvMatrix, degToRad(-yaw), [0, 1, 0]);
  mat4.translate(mvMatrix, [-xPosition-2, -yPosition-1, -zPosition-10]);
...
function animate() {
  ...
  if (lastTime != 0) {
    ...
    if (speed != 0) {
      ...
      joggingAngle += elapsed * 0.0; // 0.6 "fiddle factor" - makes it feel more realistic :-)
...
function handleKeys() {
  ...
  if ((currentlyPressedKeys[37] || currentlyPressedKeys[65])) {
    // Left cursor key or A (and forward)
    yawRate = 0.1;
  } else if ((currentlyPressedKeys[39] || currentlyPressedKeys[68])) {
    // Right cursor key or D (and forward)
    yawRate = -0.1;
  // } else if ((currentlyPressedKeys[37] || currentlyPressedKeys[65]) && (currentlyPressedKeys[40] || currentlyPressedKeys[83])) {
  //   //left and backwards
  //   yawRate = -0.05;
  // } else if ((currentlyPressedKeys[39] || currentlyPressedKeys[68]) && (currentlyPressedKeys[40] || currentlyPressedKeys[83])) {
  //   //right and backwards
  //   yawRate = 0.05;
  }else {
    yawRate = 0;
...
function start() {
  ...
  if (gl) {
    gl.clearColor(150/255.0, 220/255.0, 255/255.0, 1.0);    // Set clear color to light blue, fully opaque
